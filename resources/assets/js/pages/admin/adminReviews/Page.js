														...
														<Collapse accordion={accordion} onChange={this.onCollapseChange} activeKey={activeKey}>
															{Object.keys(reviews).map((key, i) => (
																<Panel header={reviews[key].title} key={i}>
																	...
																	{lang == 'no' && <Form.Input fluid label={translate('card.description')} name='description' placeholder={translate('card.description')} className='input-form' value={reviews[key].no_description} onChange={(val) => ref.handleChange(val, reviews[key].id + '_no_description')} />}
