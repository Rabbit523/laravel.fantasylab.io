...
class Page extends React.Component {
	...
	componentDidMount() {
		...
		Http.post(`${window.location.origin}/api/front/get-page`, { name: 'service-ui' }).then(
			res => {
														...
														{data.starting.data.map((item, i) => (
															<React.Fragment key={i}>
																<Grid.Column mobile={16} tablet={8} only="mobile" onClick={(event) => this.triggerModal(event)}>
																	<ServiceItem from='service' avatar={item.url} backimage={item.backimage} color={item.color} title={lang == 'en' ? item.title : item.no_title} description={lang == 'en' ? item.description : item.description} />
																</Grid.Column>
