...
class Page extends React.Component {
	...
	componentDidMount() {
		...
		Http.post(`${window.location.origin}/api/front/get-page`, { name: 'service-mobile' })
			.then(
				res => {
	...
	render() {
		...
		if (lang=='nb' && !window.location.pathname.includes('no')) {
			return (
				<Redirect to='no/mobilutvikling' />
			)
		} else if (lang == 'en' && window.location.pathname.includes('no')){
			return (
				<Redirect to='/mobile-development' />
			)
		}
		return (
			<Translate>
									...
									<div className='header-gradient mobile'>
										...
										<div className='starter-group'>
											<Container className='custom-col-6'>
												...
												<Button className='primary-button' onClick={(event) => this.triggerModal(event)} style={{ marginTop: 20 }}>{translate('navigation.craft-enterprise')} <Icon name='arrow right'></Icon></Button>
											</Container>
										</div>
									</div>
								</div>
