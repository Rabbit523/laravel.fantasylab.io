...
class Page extends React.Component {
	...
	componentDidMount() {
		...
		Http.post(`${window.location.origin}/api/front/get-page`, { name: 'service-web' })
			.then(
				res => {
					...
					<div className='service-page'>
						{isLoaded ?
							<React.Fragment>
								...
								<div className='service-section review' style={{ backgroundImage: `url(${data.study.backimage})` }}>
									<Container className='custom-col-6'>
												...
												{data.technologies.map((item, i) => (
													<React.Fragment key={i}>
														<Grid.Column mobile={8} tablet={8} computer={4}>
															<GuideCard from='service_web' avatar={item.icon} title={item.lang} description={lang == 'en' ? item.text : item.no_text} />
														</Grid.Column>
													</React.Fragment>
												))}
											</Grid>
